...
class NotificationManagerService:
    ...
    def __init__(
        self, notification_repository: NotificationRepositoryInterface
    ):
        """
    ...
    async def create_user_pending_notification(
        self, user: UserEnhanced, admin_ids: Optional[List[str]] = None
    ) -> None:
        """
        ...
        try:
            ...
            logger.info(
                f"Pending approval notification created for user {user.id}"
            )
    ...
    async def get_notifications(
        ...
        try:
            if unread_only and user_id:
                notifications = (
                    await self.notification_repository.get_unread_for_user(
                        user_id, limit, offset
                    )
                )
                total = await self.notification_repository.count_unread(
                    user_id
                )
            elif user_id:
                notifications = (
                    await self.notification_repository.get_all_for_user(
    ...
    async def get_badge_data(
        ...
        try:
            ...
            badge_data = await self.notification_repository.get_badge_data(
                user_id
            )
    ...
    async def mark_as_read(
        self, user_id: str, request: MarkAsReadRequest
    ) -> MarkAsReadResponse:
        """
