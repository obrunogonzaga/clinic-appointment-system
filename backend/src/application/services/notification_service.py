...
class NotificationService:
    ...
    async def send_approval_notification(self, user: UserEnhanced) -> bool:
        ...
        if self.mock_mode:
            ...
            logger.info(f"[MOCK] Subject: Cadastro Aprovado - Bem-vindo!")
            logger.info(
    ...
    async def send_rejection_notification(
        ...
        if self.mock_mode:
            ...
            logger.info(f"[MOCK] Subject: Cadastro Não Aprovado")
            logger.info(
                f"[MOCK] Body: Olá {user.name},\n\n"
    ...
    async def send_email_verification(
        ...
        if self.mock_mode:
            ...
            logger.info(f"[MOCK] Sending email verification to: {user.email}")
            logger.info(f"[MOCK] Subject: Verifique seu Email")
            logger.info(
                f"[MOCK] Body: Olá {user.name},\n\n"
    ...
    async def send_password_reset(
        ...
        if self.mock_mode:
            ...
            logger.info(f"[MOCK] Sending password reset to: {user.email}")
            logger.info(f"[MOCK] Subject: Redefinir Senha")
            logger.info(
                f"[MOCK] Body: Olá {user.name},\n\n"
    ...
    async def send_account_locked_notification(
        ...
        if self.mock_mode:
            ...
            logger.info(f"[MOCK] Subject: Conta Bloqueada Temporariamente")
            logger.info(
                f"[MOCK] Body: Olá {user.name},\n\n"
    ...
    def get_admin_emails(self) -> List[str]:
        ...
        admin_emails_str = getattr(
            self.settings, "admin_notification_emails", "admin@sistema.com"
        )
