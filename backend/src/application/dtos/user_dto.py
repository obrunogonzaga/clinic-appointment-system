...
class PublicUserRegisterRequest(BaseModel):
    ...
    role: UserRole = Field(
        UserRole.COLABORADOR, description="Requested role (cannot be ADMIN)"
    )
...
class UserEnhancedResponse(BaseModel):
    ...
    created_by: Optional[str] = Field(
        None, description="Creator user ID (null = self-registered)"
    )
...
class PendingUsersResponse(BaseModel):
    ...
    users: List[UserEnhancedResponse] = Field(
