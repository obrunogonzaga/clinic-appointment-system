...
class RateLimiter:
    ...
    def __init__(
        self, settings: Settings, redis_service: Optional[RedisService] = None
    ):
        """
        ...
        self.login_rate_limit = getattr(
            settings, "login_rate_limit", "5/minute"
        )
        self.register_rate_limit = getattr(
            settings, "register_rate_limit", "3/hour"
        )
        self.api_rate_limit = getattr(settings, "api_rate_limit", "100/minute")
        self.verification_rate_limit = getattr(
            settings, "verification_rate_limit", "2/hour"
        )
    ...
    async def check_rate_limit(
        ...
        if self.redis_service and self.redis_service.is_available():
            ...
            current = await self.redis_service.increment(
                f"rate_limit:{key}", expire=window
            )
    ...
    def enable(self):
        ...
        if self.redis_service and self.redis_service.is_available():
            storage_uri = getattr(
                self.settings, "redis_url", "redis://localhost:6379"
            )
