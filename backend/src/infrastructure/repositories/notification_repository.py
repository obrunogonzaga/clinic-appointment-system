...
class NotificationRepository(NotificationRepositoryInterface):
    ...
    async def create_indexes(self) -> None:
        ...
        indexes = [
            ...
            IndexModel(
                [("created_at", DESCENDING)], name="created_at_desc_idx"
            ),
            IndexModel(
                [("expires_at", 1)], sparse=True, name="expires_at_idx"
            ),
    ...
    async def get_unread_for_user(
        ...
        try:
            ...
            raise DomainException(
                f"Failed to get unread notifications: {str(e)}"
            )
    ...
    async def get_by_type(
        ...
        try:
            cursor = (
                self.collection.find({"type": notification_type.value})
                .sort("created_at", -1)
                .skip(offset)
                .limit(limit)
            )
            ...
            raise DomainException(
                f"Failed to get notifications by type: {str(e)}"
            )
    ...
    async def get_badge_data(
        ...
        try:
            ...
            high_priority_count = await self.collection.count_documents(
                high_priority_query
            )
