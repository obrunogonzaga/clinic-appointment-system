...
class UserApproval(BaseModel):
    ...
    approved_by: Optional[str] = Field(
        None, description="Admin user ID who approved"
    )
    ...
    rejected_by: Optional[str] = Field(
        None, description="Admin user ID who rejected"
    )
    rejected_at: Optional[datetime] = Field(
        None, description="Rejection timestamp"
    )
...
class UserSecurity(BaseModel):
    ...
    email_verification_token: Optional[str] = Field(
        None, description="Email verification token (hashed)"
    )
    ...
    failed_login_attempts: int = Field(
        0, description="Failed login attempts counter"
    )
    last_failed_attempt: Optional[datetime] = Field(
        None, description="Last failed login timestamp"
    )
    account_locked_until: Optional[datetime] = Field(
        None, description="Account lock expiry"
    )
    last_successful_login: Optional[datetime] = Field(
        None, description="Last successful login"
    )
    last_login_ip: Optional[str] = Field(
        None, description="Last login IP address"
    )

    password_reset_token: Optional[str] = Field(
        None, description="Password reset token (hashed)"
    )
    password_reset_expires: Optional[datetime] = Field(
        None, description="Password reset token expiry"
    )
    ...
    refresh_token: Optional[str] = Field(
        None, description="Current refresh token (hashed)"
    )
    refresh_token_expires: Optional[datetime] = Field(
        None, description="Refresh token expiry"
    )
    refresh_token_family: Optional[str] = Field(
        None, description="Token family for rotation tracking"
    )
...
class UserMetadata(BaseModel):
    ...
    drivers_license: Optional[str] = Field(
        None, description="Driver's license number"
    )
    license_expiry: Optional[datetime] = Field(
        None, description="License expiry date"
    )
    vehicle_assigned: Optional[str] = Field(
...
class UserEnhanced(BaseModel):
    ...
    status: UserStatus = Field(
        UserStatus.PENDENTE, description="Account status"
    )

    # Approval information
    approval: UserApproval = Field(
        default_factory=UserApproval, description="Approval information"
    )

    # Security settings
    security: UserSecurity = Field(
        default_factory=UserSecurity, description="Security settings"
    )

    # Additional metadata
    metadata: UserMetadata = Field(
        default_factory=UserMetadata, description="Additional user information"
    )
    ...
    created_by: Optional[str] = Field(
        None,
        description="User ID who created this account (null = self-registered)",
    )
    updated_by: Optional[str] = Field(
        None, description="User ID who last updated this account"
    )

    # Backward compatibility
    is_admin: bool = Field(
        False, description="Legacy admin flag (use role instead)"
    )
    is_active: bool = Field(
        True, description="Legacy active flag (use status instead)"
    )
    ...
    def increment_failed_login(self, max_attempts: int = 5) -> None:
        ...
        if self.security.failed_login_attempts >= max_attempts:
            ...
            self.security.locked_until = datetime.utcnow() + timedelta(
                minutes=30
            )
    ...
    def generate_email_verification_token(self) -> str:
        ...
        self.security.email_verification_expires = (
    ...
    def generate_password_reset_token(self) -> str:
        ...
        self.security.password_reset_expires = datetime.utcnow() + timedelta(
            hours=1
        )
        return token
