...
class TokenService:
    ...
    def create_refresh_token(
        self, user_id: str, token_family: Optional[str] = None
        ...
        """
        now_utc = datetime.now(timezone.utc)
        expires_at = now_utc + timedelta(days=self.refresh_token_expire_days)
    ...
    def verify_email_token(
        self, token: str, stored_hash: str, expiration: datetime
    ) -> bool:
        """
    ...
    def _hash_token(self, token: str) -> str:
        ...
        return hashlib.sha256(f"{token}{self.secret_key}".encode()).hexdigest()
    ...
    def invalidate_token_family(self, family_id: str):
        ...
        logger.warning(
            f"Token family {family_id} invalidated due to potential breach"
        )
